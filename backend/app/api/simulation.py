    ...
    try:
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Don't get edge info, speed things up
            )
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task doesn't exist, but if simulation_id provided, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates the reports directory to find reports matching the simulation_id.
    If multiple exist, returns the newest (sorted by created_at).
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Reads file directly, bypassing SimulationManager
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it will be stopped first and run logs cleaned
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities during simulation (posting, commenting, liking, etc.) are updated to the Zep graph in real-time
...
def close_simulation_env():
    ...
    Send a close environment command to the simulation, making it gracefully exit command waiting mode.
