...
- Each entity is an "account" or "agent" that can post, interact, and disseminate information on social media
...
- Various people appear in the text, such as "elementary school teacher", "passerby", "some netizen"
...
- Person: Any natural person (used when not matching the above specific types)
...
class OntologyGenerator:
    ...
    def __init__(self, llm_client: Optional[LLMClient] = None):
        self.llm_client = llm_client or LLMClient(role="simulation")
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(Original text has {original_length} characters in total; the first {self.MAX_TEXT_LENGTH_FOR_LLM} characters have been extracted for ontology analysis)..."
